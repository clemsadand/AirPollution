...
try:
    ...
    plt.subplot(2, 2, 1)
    for method in ['PINN', 'CRBE']:
        method_data = df_results[df_results['method'] == method]
        avg_error = method_data.groupby('time_budget')['rel_l2_error'].mean()
        plt.plot(avg_error.index, avg_error.values, marker='o', label=method)
    plt.xlabel('Time Budget (s)')
    plt.ylabel('Average Relative L2 Error')
    plt.title('Error vs Time Budget')
    plt.legend()
    plt.yscale('log')
    
    # Plot 2: Error vs Mesh Size (for largest time budget)
    plt.subplot(2, 2, 2)
    max_budget = max(time_budgets)
    max_budget_data = df_results[df_results['time_budget'] == max_budget]
    for method in ['PINN', 'CRBE']:
        method_data = max_budget_data[max_budget_data['method'] == method]
        plt.plot(method_data['mesh_size'], method_data['rel_l2_error'], 
                marker='o', label=f'{method} ({max_budget}s)')
    plt.xlabel('Mesh Size')
    plt.ylabel('Relative L2 Error')
    plt.title(f'Error vs Mesh Size (Time Budget: {max_budget}s)')
    plt.legend()
    plt.yscale('log')
    plt.xscale('log')
    ...
    plt.subplot(2, 2, 3)
    pinn_runtimes = df_results[df_results['method'] == 'PINN']['actual_runtime']
    crbe_runtimes = df_results[df_results['method'] == 'CRBE']['actual_runtime']
    plt.hist(pinn_runtimes, alpha=0.7, label='PINN', bins=20)
    plt.hist(crbe_runtimes, alpha=0.7, label='CRBE', bins=20)
    ...
    for method in ['PINN', 'CRBE']:
        method_data = df_results[df_results['method'] == method]
        avg_memory = method_data.groupby('time_budget')['cpu_memory_usage_MB'].mean()
        plt.plot(avg_memory.index, avg_memory.values, marker='o', label=f'{method} CPU')
    plt.xlabel('Time Budget (s)')
    plt.ylabel('Average CPU Memory Usage (MB)')
    plt.title('Memory Usage vs Time Budget')
    plt.legend()
