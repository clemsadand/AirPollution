...
class ComprehensiveAnalysis:
    ...
    def compute_mass_conservation(self):
        """Analyze mass conservation over time for both methods"""
        ...
        # Get mesh properties for integration
        midpoints = torch.tensor(self.mesh_data.midpoints, dtype=torch.float32, device=device)
        areas = self.mesh_data.segment_lengths  # Assuming this exists in mesh_data
        
        for i, t in enumerate(times):
            # CRBE mass at time t
            crbe_solution = self.solver_crbe.solutions[i, :]
            crbe_mass = np.sum(crbe_solution * areas)
            ...
            pinn_mass = np.sum(pinn_solution * areas)
    ...
    def compute_center_of_mass_tracking(self):
        """Track the center of mass movement for both methods"""
        print("Computing center of mass tracking...")
        
        times = self.mesh_data.time_discr
        midpoints = torch.tensor(self.mesh_data.midpoints, dtype=torch.float32, device=device)
        areas = self.mesh_data.segment_lengths
        ...
        for i, t in enumerate(times):
            # CRBE center of mass
            crbe_solution = self.solver_crbe.solutions[i, :]
            total_mass = np.sum(crbe_solution * areas)
            
            if total_mass > 1e-10:  # Avoid division by zero
                com_x = np.sum(crbe_solution * areas * self.mesh_data.midpoints[:, 0]) / total_mass
                com_y = np.sum(crbe_solution * areas * self.mesh_data.midpoints[:, 1]) / total_mass
            else:
                com_x, com_y = 0, 0
            
            crbe_com_x.append(com_x)
            crbe_com_y.append(com_y)
            ...
            total_mass_pinn = np.sum(pinn_solution * areas)
            
            if total_mass_pinn > 1e-10:
                com_x_pinn = np.sum(pinn_solution * areas * self.mesh_data.midpoints[:, 0]) / total_mass_pinn
                com_y_pinn = np.sum(pinn_solution * areas * self.mesh_data.midpoints[:, 1]) / total_mass_pinn
            else:
    ...
    def compute_spreading_rate_analysis(self):
        """Analyze the spreading rate (second moments) of the plume"""
        ...
        for i, t in enumerate(times):
            ...
            if total_mass > 1e-10:
                ...
                # Second moments
                var_x = np.sum(crbe_solution * areas * (self.mesh_data.midpoints[:, 0] - com_x)**2) / total_mass
                var_y = np.sum(crbe_solution * areas * (self.mesh_data.midpoints[:, 1] - com_y)**2) / total_mass
            else:
                var_x, var_y = 0, 0
            
            crbe_var_x.append(var_x)
            crbe_var_y.append(var_y)
            
            # PINN variance calculation
            t_tensor = torch.full((midpoints.shape[0], 1), t, dtype=torch.float32, device=device)
            xyt = torch.cat([midpoints, t_tensor], dim=1)
            ...
            if total_mass_pinn > 1e-10:
                ...
                var_x_pinn = np.sum(pinn_solution * areas * (self.mesh_data.midpoints[:, 0] - com_x_pinn)**2) / total_mass_pinn
                var_y_pinn = np.sum(pinn_solution * areas * (self.mesh_data.midpoints[:, 1] - com_y_pinn)**2) / total_mass_pinn
            else:
